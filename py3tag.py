...
# Load an audio buffer using audioread (modified from librosa, originally ISC licensed)
...
# Update mp3 tags
def mp3_tag(mp3_filename, artist, album, track, tracks, title, year, genre, bpms, compilation):
...
for mp3_filename in mp3_files:
    ...
    try:
        with audioread.audio_open(mp3_filename) as f:
            # Test mp3 file
            ...
            if (len(mp3_info) == 4):
                ...
                if (__DRY_RUN__ == False):
                    try:
                        mp3_tag(mp3_filename, artist, album, track, tracks, title, year, genre, bpms, compilation)
                    except:
